	...
	while(ptoken = dequeue(pqueue_infix))
	{
		switch(ptoken->type)
		{
			...
			case RPARENS:
				...
				*/
				p_expr_token ptoken_temporary = NULL;
				while ((ptoken_temporary = pop(&stack_top))->type != LPARENS)
					enqueue(&queue_postfix, ptoken_temporary);
				/* stack top is LPARENS */
				pop(&stack_top);
				break;
		}
...
double evaluate_postfix(struct token_queue * pqueue_postfix) 
{
	...
	/* init operands stack (unary/binary) */
	...
	while(ptoken = dequeue(pqueue_postfix))
	{
		...
		/* if not an operand: ptoken->type == OPERATOR */
		double result = 0;
		...
		{
			case ADD:
				right_operand = (pop(&stack_top))->value.operand;
				left_operand = (pop(&stack_top))->value.operand;
			...
			case DIVIDE:
				...
				if (right_operand == 0)
				{
					printf("[DIVIDE by ZERO ERROR] %f / %f\n", left_operand, right_operand);
					abort();
				}
				result = left_operand / right_operand;
				break;
		...
		/* push the result of operands evaluated back to stack */
		union token_value value;
		value.operand = result;
		p_expr_token ptoken_result = new_token(OPERAND, value);
		push(&stack_top, ptoken_result);
	}

	/* all tokens dequeued from postfix queue */
	/* result is the final single token exist in stack */
	return stack_top->value.operand;
}
...
void display_queue(struct token_queue* pqueue)
{
	...
	printf("Queue: { ");
	if(i==NULL)
		printf("NULL Pointer. Empty Queue");
	while(i!=NULL)
	{
		if(i->type == OPERAND)
		{
