...
#if DEBUG
void display_queue(struct token_queue* ptq);
#endif
...
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) 
{
    ...
    while( ptoken = dequeue(pqueue_infix) )
    {
        ...
        {
            precedence_top = op_precedences[((stack_top)->value).op_code];
            precedence_token = op_precedences[(ptoken->value).op_code];
            condition1 = (int)(precedence_top > precedence_token);
            condition2 = (int)((precedence_top==precedence_token));

            while(stack_top && condition1 || condition2 && (op_associativity[precedence_top]==LEFT))
            {
                p_expr_token ptop_op = pop(&stack_top);
                enqueue(&queue_postfix, ptop_op);
            }
...
void display_queue(struct token_queue* ptq)
{
    while((*ptq).front)
    {
        if ((((*ptq).front)->type)==OPERATOR)
        {
            switch ((((*ptq).front)->value).op_code)
            {
            case 0:
                printf(" %c ", '+');
                break;
        ...
        double val = ((*ptq).front)->value.operand;
        printf(" %.f ",val);
        ((*ptq).front) = ((*ptq).front)->linked_token;
    }
    puts("disp end");
}