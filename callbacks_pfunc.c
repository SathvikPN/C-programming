  ...
  for(i=0;i<nrec;i++)
    {
      /*callback*/
      fp(&sarr[i],arg);
      /* need to match no. params for each possible function
       * here, fp can be {printrec, isolder}
       * isolder need two param but one param enough for printrec
       * Same params are passed to all possible functions assumed to be passed
       */
    }
}
...
{
  struct student *pstudent = (struct student *)prec;
  int *threshold_age = (int *)arg;
  if(pstudent->age > (*threshold_age))
    printrec(pstudent, NULL);
}
